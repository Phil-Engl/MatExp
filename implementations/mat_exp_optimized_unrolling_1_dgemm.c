...
int is_sparse(const double * A, int m, int n){
    ...
    for(int i=0; i<m*n; i++){
        ...
        FLOP_COUNT_INC(1, "is_sparse");
        if(A[i] != 0){
            count++;
        }
    }
    
    FLOP_COUNT_INC(3, "is_sparse");
 ...
 // ikj
    ...
    for(int i=0; i<n; i++){
        for(k=0; k<n-3; k+=4){
            ...
            FLOP_COUNT_INC(4, "forward_substitution");
            ...
            for(; j<i; j++){
                ...
                FLOP_COUNT_INC(8, "forward_substitution");
            ...
            FLOP_COUNT_INC(6*4, "forward_substitution");
            //store result
 ...
 // ikj
    ...
    for(int i=n-1; i>=0; i--){
        for(k=0; k<n-3; k+=4){
            ...
            FLOP_COUNT_INC(4, "backward_substitution");
            ...
            FLOP_COUNT_INC(8, "backward_substitution");
            ...
            FLOP_COUNT_INC(6*4, "backward_substitution");
        ...
        for(; k<n; k++){
            double sum = 0;
                ...
                FLOP_COUNT_INC(3, "backward_substitution");
            }
        }
    ...
    for(int i=n-1; i>=0; i--){
        for(k=0; k<n-3; k+=4){
            ...
            for(j=i+1; j<n-3; j+=4){
                ...
                FLOP_COUNT_INC(4*4*2, "backward_substitution_LU");
            }
            //CLEANUP LOOP FOR J
            for(; j<n; j++){
           // __m256d x_vec_cleanup= _mm256_loadu_pd( &x[j*n+k] );
            ...
            FLOP_COUNT_INC(8, "backward_substitution_LU");
            ...
            FLOP_COUNT_INC(5*4, "backward_substitution_LU");
        ...
        for(; k<n; k++){
            double sum = 0;
                for(j=i+1; j<n; j++){
                    ...
                    FLOP_COUNT_INC(2, "backward_substitution_LU");;
                }
                x[i*n + k] = b[i*n+k] - sum;
                
                FLOP_COUNT_INC(1, "backward_substitution_LU");
                //x[i*n + k] /= U[i*n + i];
            }
        }
       ...
       int i;
        ...
        for(int i=0; i<(n-k-1); i++){
            ...
            for(j=0; j<(n-k-1)-3; j+=4){
                ...
                FLOP_COUNT_INC(2*4, "LU_decomposition");
...
double onenorm_abs_mat(const double* A, int m, int n){
    ...
    for(int i = 0; i < n; i+=1){ 
        ...
        for(int j = 0; j < m; j+=4){
            ...
            FLOP_COUNT_INC(4, "onenorm_abs_mat");
    ...
    do{
        ...
        for(int m = 7; m <= 9; m+=2){
            ...
            if(eta_3 <= theta[m] && ell(A_abs_pow_m, normA, n, m) == 0){
                ...
                FLOP_COUNT_INC(4, "mat_exp p=7,9");
        ...
        FLOP_COUNT_INC(12, "mat_exp p=13"); // TODO: pow(2, -s) = 1 flop? (s is an integer)
