...
#define EPS (1e-3)
#define TEST_SIZE (8)
#define MIN_NUM_RUNS 30

string flop_count_file = "PATH_TO_FLOP_COUNT_FILE_PLACEHOLDER";
string benchmark_filename = "PATH_TO_BENCHMARK_OUTPUT_FILE_PLACEHOLDER";
 
 // Sorting for median instead of average
  ...
  for(size_t mat = 0; mat < num_tests; mat++){
    ...
    */
    ...
    for (int i = 0; i < numFuncs; i++) {
      ...
      if(i == 1 && is_col_major){
        transpose_matrix(input_matrix, input_size);
      }
    ...
    */
      if(i == 1 && is_col_major){
        transpose_matrix(out_mat1, input_size);
      }
...
double perf_test_single(comp_func f, string desc, int flops){
    ...
    double * median_time = (double *) malloc(REP*sizeof(double));
    for (size_t j = 0; j < REP; j++) {
        ...
        median_time[j] = cycles;
        total_cycles += cycles;

        cyclesList.push_back(cycles);
    }
    total_cycles /= REP;
    sort(median_time, REP);
    double return_time = median_time[REP/2];
    cycles = total_cycles;
    free(median_time);
    free(out_mat);
    return  return_time;
