...
void mat_exp(const double *A, int n, double *E){
    ...
    if(n % 4 != 0){
        printf("n has to be a multiple of 4. Abort.\n");
        exit(-1);
    }
    ...
    do{
        ...
        if(eta_1 <= theta[3] && ell(A, n, 3) == 0){
            eval3_4(A, A_2, A_4, A_6, A_6, n, 3, P_m, Q_m);
        ...
        if(eta_2 <= theta[5] && ell(A, n, 5) == 0){
            ...
            eval3_4(A, A_2, A_4, A_6, A_6, n, 5, P_m, Q_m);
        ...
        for(int m = 7; m <= 9; m+=2){
            if(eta_3 <= theta[m] && ell(A, n, m) == 0){
                ...
                eval3_4(A, A_2, A_4, A_6, A_8, n, m, P_m, Q_m);
        ...
        if(infinity_norm(Temp, n, 1)/divider <= theta13_exp_10){
            ...
            // TODO: FLOP_COUNT_INC pow(2, -s1) = 1 flop? (s is an integer)
