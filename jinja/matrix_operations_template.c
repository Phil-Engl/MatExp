...
double infinity_norm(const double* A, int m, int n){
    ...
    double max = 0.0;
    {% for i in range(r) %}
    __m256d max_val_{{ i }} = _mm256_set1_pd(0.0);
    {% endfor %}
    for(int i = 0; i < m; i+=4 * {{ r }}){
        ...
        for(int j = 0; j < n; j++){
            {% for i in range(r) %}
            __m256d ld_{{ i }} = _mm256_load_pd(&A[j * m + i + 4 * {{ i }}]);
            {% endfor %}
            {% for i in range(r) %}
            sum_{{ i }} = _mm256_add_pd(sum_{{ i }}, ld_{{ i }});
            {% endfor %}
        }
        {% for i in range(r) %}
        max_val_{{ i }} = _mm256_max_pd(max_val_{{ i }}, sum_{{ i }});
        {% endfor %}
    }
    for(int i = 0; i < 4; i++){
        {% for i in range(r) %}
        max = max_val_{{ i }}[i] > max ? max_val_{{ i }}[i] : max;
        {% endfor %}
    }
    return max;
}
