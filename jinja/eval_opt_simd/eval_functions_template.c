...
{   
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j+=4*{{l}}){
            ...
            {% for i in range(l) %}
            __m256d u_init_{{i}} = _mm256_set_pd((i==j+3+ 4*{{i}}? pade_coefs[1] : 0.0), (i==j+2+ 4*{{i}}? pade_coefs[1] : 0.0), (i==j+1+ 4*{{i}}? pade_coefs[1] : 0.0), (i==j+ 4*{{i}}? pade_coefs[1] : 0.0));
            __m256d v_init_{{i}} = _mm256_set_pd((i==j+3+ 4*{{i}}? pade_coefs[0] : 0.0), (i==j+2+ 4*{{i}}? pade_coefs[0] : 0.0), (i==j+1+ 4*{{i}}? pade_coefs[0] : 0.0), (i==j+ 4*{{i}}? pade_coefs[0] : 0.0));
            {% endfor %}

            {% for i in range(l) %}
            __m256d u_tmp_{{i}} = _mm256_fmadd_pd(pade_3, a_{{i}}, u_init_{{i}});
            __m256d v_tmp_{{i}} = _mm256_fmadd_pd(pade_2, a_{{i}}, v_init_{{i}});
            {% endfor %}
...
{       
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j+=4*{{l}}){
            ...
            {% for i in range(l) %}
            __m256d u_tmp_{{i}} = _mm256_fmadd_pd(pade_3, a2_{{i}}, u_init_{{i}});
            __m256d v_tmp_{{i}} = _mm256_fmadd_pd(pade_2, a2_{{i}}, v_init_{{i}});
...
{       
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j+=4*{{l}}){
            ...
            {% for i in range(l) %}
            __m256d u_tmp1_{{i}} = _mm256_fmadd_pd(pade_3, a2_{{i}}, u_init_{{i}});
            __m256d v_tmp1_{{i}} = _mm256_fmadd_pd(pade_2, a2_{{i}}, v_init_{{i}});
