...
double onenormest(const double* A, int n, int t, int itmax, int get_v, int get_w, double* v, double* w){
    ...
    int* ind_hist = (int*)aligned_alloc(32,n * itmax * sizeof(int));
    ...
    double *S_T = (double*)aligned_alloc(32,n * t * sizeof(double));
    double *S_old_T = (double*)aligned_alloc(32,n * t * sizeof(double));
    ...
    struct h_tmp* h = (struct h_tmp*)aligned_alloc(32,n * sizeof(struct h_tmp));
...
double normest(const double* A, int n){
    ...
    double* dummyv = (double*)aligned_alloc(32,sizeof(double));
    double* dummyw = (double*)aligned_alloc(32,sizeof(double));
    int t = 1;
    if(n > 2){
        t = 2;
    }
