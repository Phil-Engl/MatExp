...
#include "../base_implementation/matrix_operations.h"
#include "../base_implementation/matrix_exponential.h"
#include "../base_implementation/onenorm.h"
...
double eval3_4_benchmark(const double* A, const double* A_2, const double* A_4, const double* A_6, int n, const int m, double *P_9, double *Q_9){
    double cycles = 0.;
    long num_runs = 100;
    double multiplier = 1;
    myInt64 start, end;
    ...
    // Warm up phase
     do {
        ...
        double max_val;
        for (long i = 0; i < num_runs; i++) {
            eval3_4(A, A_2, A_4, A_6, n, m, P_9, Q_9);      
        }
        end = stop_tsc(start);
...
int main (int argc, char* argv[]) {
    ...
    if(argc < 10){
        printf("mmm mm_add onenorm normest eval3_4 eval3_5 eval3_6\n");
        return 0;
    }
    int mmm_on = atoi(argv[3]);
    int scm_on = atoi(argv[4]);
    int onenorm_on = atoi(argv[5]);
    int normest_on = atoi(argv[6]);
    int eval3_4_on = atoi(argv[7]);
    int eval3_5_on = atoi(argv[8]);
    int eval3_6_on = atoi(argv[9]);
    ...
    int normest_fc[] = {13922, 52549, 203394, 800002, 3172868, 7118598, 12637188};
    ...
    for(int i = lower; i < upper; i++){
        ...
        if(normest_on){
            res = onenormest_benchmark(A,n);
            flops = normest_fc[i];
            printf("normest, %.4f, %4d, %.4f\n", res, n, flops/res);
        }
        ...
        if(eval3_4_on){
            flops = 21*n*n + 4*n*n*n;
            res = eval3_4_benchmark(A, A_2, A_4, A_6, n, 9, P_m, Q_m);
            printf("eval3_4, %.4f, %4d, %.4f\n", res, n, flops/res);
            
        }

        if(eval3_5_on){
            flops = 31*n*n + 6*n*n*n;
            res = eval3_5_benchmark(A, A_2, A_4, A_6, n, P_m, Q_m);
            printf("eval3_5, %.4f, %4d, %.4f\n", res, n, flops/res);        
        }

        if(eval3_6_on){
            flops = 3*n*n*n + 2*n*n*n;
            res = eval3_6_benchmark(A,B,n,C,0);
            printf("eval3_6, %.4f, %4d, %.4f\n", res, n, flops/res);  
        }
