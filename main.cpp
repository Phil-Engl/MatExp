...
string flop_count_file = "implementations/matrix_exponential_unrolled_1_with_blas";
string benchmark_filename = "implementations/matrix_exponential_unrolled_1_with_blas";
...
void read_flop_counts(vector<string> *matrices_flop_count, vector<int>* flop_counts){
  ifstream inFile;
...
int get_flop_count(string matrix_name, vector<string> matrices_flop_count, vector<int> flop_counts){
...
void write_results_to_file(double* runtime_results, double* perf_results, double* flop_counts){
  ofstream file;
...
void perf_test() {
  ...
  double* flops_output = (double *) aligned_alloc(32, numFuncs*num_tests*sizeof(double));
  ...
  for(int s = 0; s < num_tests; s++){
    ...
    int flops = get_flop_count(get_matrix_name(s), matrices_flop_count, flop_counts);
