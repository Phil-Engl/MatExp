...
void mm_add(double alpha, const double *A, const double *B, double *C, int m, int n){
    ...
    for(int i = 0 ; i < m*n; i+=4 * 4){
         ...
         a_1 =_mm256_load_pd(&A[i + (1 * 4)]);
         b_1 = _mm256_load_pd(&B[i + (1 * 4)]);
         c_1 = _mm256_fmadd_pd(alph, a_1, b_1);
         _mm256_store_pd(&C[i + (1 * 4)], c_1);
        
         a_2 =_mm256_load_pd(&A[i + (2 * 4)]);
         b_2 = _mm256_load_pd(&B[i + (2 * 4)]);
         c_2 = _mm256_fmadd_pd(alph, a_2, b_2);
         _mm256_store_pd(&C[i + (2 * 4)], c_2);
        
         a_3 =_mm256_load_pd(&A[i + (3 * 4)]);
         b_3 = _mm256_load_pd(&B[i + (3 * 4)]);
         c_3 = _mm256_fmadd_pd(alph, a_3, b_3);
         _mm256_store_pd(&C[i + (3 * 4)], c_3);
...
void scalar_matrix_mult(double alpha, const double *A, double *C, int m, int n){
    ...
    for (int i = 0 ; i < m*n ; i+= 4*4){
        ...
        temp_1 = _mm256_load_pd(&A[i + (1* 4)]);
        temp_1 = _mm256_mul_pd(alph, temp_1);
        _mm256_store_pd(&C[i + (1* 4)], temp_1);
        
        temp_2 = _mm256_load_pd(&A[i + (2* 4)]);
        temp_2 = _mm256_mul_pd(alph, temp_2);
        _mm256_store_pd(&C[i + (2* 4)], temp_2);
        
        temp_3 = _mm256_load_pd(&A[i + (3* 4)]);
        temp_3 = _mm256_mul_pd(alph, temp_3);
        _mm256_store_pd(&C[i + (3* 4)], temp_3);
...
void mat_abs(const double *A, double *B, int m, int n){
    ...
    for(int i = 0 ; i < m*n ; i+= 4 * 4){
        ...
        a_1 = _mm256_load_pd(&A[i + (1*4)]);
        a_1 = _mm256_andnot_pd(sign_mask, a_1);
        _mm256_store_pd(&B[i + (1 * 4)], a_1);
        
        a_2 = _mm256_load_pd(&A[i + (2*4)]);
        a_2 = _mm256_andnot_pd(sign_mask, a_2);
        _mm256_store_pd(&B[i + (2 * 4)], a_2);
        
        a_3 = _mm256_load_pd(&A[i + (3*4)]);
        a_3 = _mm256_andnot_pd(sign_mask, a_3);
        _mm256_store_pd(&B[i + (3 * 4)], a_3);
...
void mat_col_sum(const double* A, int n, double *out){
    ...
    for(int i = 0; i < n; i+=4){ 
        ...
        __m256d acc_2 = _mm256_set1_pd(0.0); 
        
        __m256d acc_3 = _mm256_set1_pd(0.0); 
        
        for(int j = 0; j < n; j+=4){
            
            __m256d ld_0 = _mm256_load_pd(&A[(i + 0) * n + j]);
            
            __m256d ld_1 = _mm256_load_pd(&A[(i + 1) * n + j]);
            
            __m256d ld_2 = _mm256_load_pd(&A[(i + 2) * n + j]);
            
            __m256d ld_3 = _mm256_load_pd(&A[(i + 3) * n + j]);
            ...
            acc_2 = _mm256_add_pd(acc_2, ld_2);
            
            acc_3 = _mm256_add_pd(acc_3, ld_3);
        ...
        res_2 = acc_2[0] + acc_2[1] + acc_2[2] + acc_2[3];
        
        res_3 = acc_3[0] + acc_3[1] + acc_3[2] + acc_3[3];
...
void fill_diagonal_matrix(double* A, double diag, int n){
    ...
    for(int i = 0; i < n*n; i+=4 * 4){
        
        _mm256_store_pd(&A[i + 4 * 0], ZERO);
        
        _mm256_store_pd(&A[i + 4 * 1], ZERO);
        
        _mm256_store_pd(&A[i + 4 * 2], ZERO);
        
        _mm256_store_pd(&A[i + 4 * 3], ZERO);
...
double onenorm(const double* A, int m, int n){
    ...
    for(int i = 0; i < n; i+=4){ 
        ...
        for(int j = 0; j < m; j+=4){
            ...
            __m256d ld_2 = _mm256_load_pd(&A[(i + 2) * m + j]);
            
            __m256d ld_3 = _mm256_load_pd(&A[(i + 3) * m + j]);
            ...
            ld_2 = _mm256_andnot_pd(ABS_MASK, ld_2);
            
            ld_3 = _mm256_andnot_pd(ABS_MASK, ld_3);
...
{   
    ...
    for(int i=0; i<n*n; i+=4*4){
        ...
        __m256d u_tmp_1 = _mm256_load_pd(&Temp[i + 4*1]);
        __m256d v_tmp_1 = _mm256_load_pd(&V[i + 4*1]);
        
        __m256d u_tmp_2 = _mm256_load_pd(&Temp[i + 4*2]);
        __m256d v_tmp_2 = _mm256_load_pd(&V[i + 4*2]);
        
        __m256d u_tmp_3 = _mm256_load_pd(&Temp[i + 4*3]);
        __m256d v_tmp_3 = _mm256_load_pd(&V[i + 4*3]);
        ...
        __m256d p_tmp_1 = _mm256_add_pd(u_tmp_1, v_tmp_1);
        __m256d q_tmp_1 = _mm256_sub_pd(v_tmp_1, u_tmp_1);
        
        __m256d p_tmp_2 = _mm256_add_pd(u_tmp_2, v_tmp_2);
        __m256d q_tmp_2 = _mm256_sub_pd(v_tmp_2, u_tmp_2);
        
        __m256d p_tmp_3 = _mm256_add_pd(u_tmp_3, v_tmp_3);
        __m256d q_tmp_3 = _mm256_sub_pd(v_tmp_3, u_tmp_3);
        ...
        _mm256_store_pd(&P_3[i + 4*1], p_tmp_1);
        _mm256_store_pd(&Q_3[i + 4*1], q_tmp_1);
        
        _mm256_store_pd(&P_3[i + 4*2], p_tmp_2);
        _mm256_store_pd(&Q_3[i + 4*2], q_tmp_2);
        
        _mm256_store_pd(&P_3[i + 4*3], p_tmp_3);
        _mm256_store_pd(&Q_3[i + 4*3], q_tmp_3);
...
{   
    ...
    for(int i=0; i<n*n; i+=4*4){
        ...
        _mm256_store_pd(&P_5[i + 4*1], p_tmp_1);
        _mm256_store_pd(&Q_5[i + 4*1], q_tmp_1);
        
        _mm256_store_pd(&P_5[i + 4*2], p_tmp_2);
        _mm256_store_pd(&Q_5[i + 4*2], q_tmp_2);
        
        _mm256_store_pd(&P_5[i + 4*3], p_tmp_3);
        _mm256_store_pd(&Q_5[i + 4*3], q_tmp_3);
...
{   
    ...
    for(int i=0; i<n*n; i+=4*4){
        ...
        _mm256_store_pd(&P_7[i + 4*1], p_tmp_1);
        _mm256_store_pd(&Q_7[i + 4*1], q_tmp_1);
        
        _mm256_store_pd(&P_7[i + 4*2], p_tmp_2);
        _mm256_store_pd(&Q_7[i + 4*2], q_tmp_2);
        
        _mm256_store_pd(&P_7[i + 4*3], p_tmp_3);
        _mm256_store_pd(&Q_7[i + 4*3], q_tmp_3);
...
{   
    ...
    for(int i=0; i<n*n; i+=4*4){
        ...
        _mm256_store_pd(&P_9[i + 4*1], p_tmp_1);
        _mm256_store_pd(&Q_9[i + 4*1], q_tmp_1);
        
        _mm256_store_pd(&P_9[i + 4*2], p_tmp_2);
        _mm256_store_pd(&Q_9[i + 4*2], q_tmp_2);
        
        _mm256_store_pd(&P_9[i + 4*3], p_tmp_3);
        _mm256_store_pd(&Q_9[i + 4*3], q_tmp_3);
...
void eval3_5(const double *A, double* A_2, double* A_4, double* A_6, int n, double *P_13, double *Q_13){
    ...
    for(int i = 0; i < n*n; i+=4*4){
        ...
        __m256d a6_i_1 = _mm256_load_pd(&A_6[i+4*1]);
        __m256d a4_i_1 = _mm256_load_pd(&A_4[i+4*1]);
        __m256d a2_i_1 = _mm256_load_pd(&A_2[i+4*1]);
        
        __m256d a6_i_2 = _mm256_load_pd(&A_6[i+4*2]);
        __m256d a4_i_2 = _mm256_load_pd(&A_4[i+4*2]);
        __m256d a2_i_2 = _mm256_load_pd(&A_2[i+4*2]);
        
        __m256d a6_i_3 = _mm256_load_pd(&A_6[i+4*3]);
        __m256d a4_i_3 = _mm256_load_pd(&A_4[i+4*3]);
        __m256d a2_i_3 = _mm256_load_pd(&A_2[i+4*3]);
        ...
        __m256d u_tmp0_1 = _mm256_mul_pd(b_13, a6_i_1);
        __m256d v_tmp0_1 = _mm256_mul_pd(b_12, a6_i_1);

        __m256d u_tmp1_1 = _mm256_fmadd_pd(b_11, a4_i_1, u_tmp0_1);
        __m256d v_tmp1_1 = _mm256_fmadd_pd(b_10, a4_i_1, v_tmp0_1);

        __m256d u_tmp2_1 = _mm256_fmadd_pd(b_9, a2_i_1, u_tmp1_1);
        __m256d v_tmp2_1 = _mm256_fmadd_pd(b_8, a2_i_1, v_tmp1_1);
        
        __m256d u_tmp0_2 = _mm256_mul_pd(b_13, a6_i_2);
        __m256d v_tmp0_2 = _mm256_mul_pd(b_12, a6_i_2);

        __m256d u_tmp1_2 = _mm256_fmadd_pd(b_11, a4_i_2, u_tmp0_2);
        __m256d v_tmp1_2 = _mm256_fmadd_pd(b_10, a4_i_2, v_tmp0_2);

        __m256d u_tmp2_2 = _mm256_fmadd_pd(b_9, a2_i_2, u_tmp1_2);
        __m256d v_tmp2_2 = _mm256_fmadd_pd(b_8, a2_i_2, v_tmp1_2);
        
        __m256d u_tmp0_3 = _mm256_mul_pd(b_13, a6_i_3);
        __m256d v_tmp0_3 = _mm256_mul_pd(b_12, a6_i_3);

        __m256d u_tmp1_3 = _mm256_fmadd_pd(b_11, a4_i_3, u_tmp0_3);
        __m256d v_tmp1_3 = _mm256_fmadd_pd(b_10, a4_i_3, v_tmp0_3);

        __m256d u_tmp2_3 = _mm256_fmadd_pd(b_9, a2_i_3, u_tmp1_3);
        __m256d v_tmp2_3 = _mm256_fmadd_pd(b_8, a2_i_3, v_tmp1_3);
        ...
        _mm256_store_pd(&U_tmp[i+4*1], u_tmp2_1);
        _mm256_store_pd(&V_tmp[i+4*1], v_tmp2_1);
        
        _mm256_store_pd(&U_tmp[i+4*2], u_tmp2_2);
        _mm256_store_pd(&V_tmp[i+4*2], v_tmp2_2);
        
        _mm256_store_pd(&U_tmp[i+4*3], u_tmp2_3);
        _mm256_store_pd(&V_tmp[i+4*3], v_tmp2_3);
    ...
    for(int i = 0; i < n*n; i+=4*4){
        ...
        __m256d a4_i_1 = _mm256_load_pd(&A_4[i+4*1]);
        __m256d a2_i_1 = _mm256_load_pd(&A_2[i+4*1]);
        __m256d u_tmp0_1 = _mm256_load_pd(&U[i+4*1]);
        __m256d v_tmp0_1 = _mm256_load_pd(&V[i+4*1]);
        
        __m256d a6_i_2 = _mm256_load_pd(&A_6[i+4*2]);
        __m256d a4_i_2 = _mm256_load_pd(&A_4[i+4*2]);
        __m256d a2_i_2 = _mm256_load_pd(&A_2[i+4*2]);
        __m256d u_tmp0_2 = _mm256_load_pd(&U[i+4*2]);
        __m256d v_tmp0_2 = _mm256_load_pd(&V[i+4*2]);
        
        __m256d a6_i_3 = _mm256_load_pd(&A_6[i+4*3]);
        __m256d a4_i_3 = _mm256_load_pd(&A_4[i+4*3]);
        __m256d a2_i_3 = _mm256_load_pd(&A_2[i+4*3]);
        __m256d u_tmp0_3 = _mm256_load_pd(&U[i+4*3]);
        __m256d v_tmp0_3 = _mm256_load_pd(&V[i+4*3]);
        ...
        __m256d u_tmp1_1 = _mm256_fmadd_pd(b_7, a6_i_1, u_tmp0_1);
        __m256d v_tmp1_1 = _mm256_fmadd_pd(b_6, a6_i_1, v_tmp0_1);

        __m256d u_tmp2_1 = _mm256_fmadd_pd(b_5, a4_i_1, u_tmp1_1);
        __m256d v_tmp2_1 = _mm256_fmadd_pd(b_4, a4_i_1, v_tmp1_1);

        __m256d u_tmp3_1 = _mm256_fmadd_pd(b_3, a2_i_1, u_tmp2_1);
        __m256d v_tmp3_1 = _mm256_fmadd_pd(b_2, a2_i_1, v_tmp2_1);
        
        __m256d u_tmp1_2 = _mm256_fmadd_pd(b_7, a6_i_2, u_tmp0_2);
        __m256d v_tmp1_2 = _mm256_fmadd_pd(b_6, a6_i_2, v_tmp0_2);

        __m256d u_tmp2_2 = _mm256_fmadd_pd(b_5, a4_i_2, u_tmp1_2);
        __m256d v_tmp2_2 = _mm256_fmadd_pd(b_4, a4_i_2, v_tmp1_2);

        __m256d u_tmp3_2 = _mm256_fmadd_pd(b_3, a2_i_2, u_tmp2_2);
        __m256d v_tmp3_2 = _mm256_fmadd_pd(b_2, a2_i_2, v_tmp2_2);
        
        __m256d u_tmp1_3 = _mm256_fmadd_pd(b_7, a6_i_3, u_tmp0_3);
        __m256d v_tmp1_3 = _mm256_fmadd_pd(b_6, a6_i_3, v_tmp0_3);

        __m256d u_tmp2_3 = _mm256_fmadd_pd(b_5, a4_i_3, u_tmp1_3);
        __m256d v_tmp2_3 = _mm256_fmadd_pd(b_4, a4_i_3, v_tmp1_3);

        __m256d u_tmp3_3 = _mm256_fmadd_pd(b_3, a2_i_3, u_tmp2_3);
        __m256d v_tmp3_3 = _mm256_fmadd_pd(b_2, a2_i_3, v_tmp2_3);
        ...
        _mm256_store_pd(&U[i+4*1], u_tmp3_1);
        _mm256_store_pd(&V[i+4*1], v_tmp3_1);
        
        _mm256_store_pd(&U[i+4*2], u_tmp3_2);
        _mm256_store_pd(&V[i+4*2], v_tmp3_2);
        
        _mm256_store_pd(&U[i+4*3], u_tmp3_3);
        _mm256_store_pd(&V[i+4*3], v_tmp3_3);
    ...
    for(int i=0; i < n*n; i+=4*4){
        ...
        __m256d u_tmp_1 = _mm256_load_pd(&U_tmp[i+4*1]);
        __m256d v_tmp_1 = _mm256_load_pd(&V[i+4*1]);
        
        __m256d u_tmp_2 = _mm256_load_pd(&U_tmp[i+4*2]);
        __m256d v_tmp_2 = _mm256_load_pd(&V[i+4*2]);
        
        __m256d u_tmp_3 = _mm256_load_pd(&U_tmp[i+4*3]);
        __m256d v_tmp_3 = _mm256_load_pd(&V[i+4*3]);
        ...
        _mm256_store_pd(&P_13[i+4*1], p_tmp_1);
        _mm256_store_pd(&Q_13[i+4*1], q_tmp_1);
        
        _mm256_store_pd(&P_13[i+4*2], p_tmp_2);
        _mm256_store_pd(&Q_13[i+4*2], q_tmp_2);
        
        _mm256_store_pd(&P_13[i+4*3], p_tmp_3);
        _mm256_store_pd(&Q_13[i+4*3], q_tmp_3);
